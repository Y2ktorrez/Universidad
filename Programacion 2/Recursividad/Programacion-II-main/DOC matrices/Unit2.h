//---------------------------------------------------------------------------

#ifndef Unit2H
#define Unit2H
//---------------------------------------------------------------------------
#include <System.Classes.hpp>
#include <Vcl.Controls.hpp>
#include <Vcl.StdCtrls.hpp>
#include <Vcl.Forms.hpp>
#include <Vcl.Grids.hpp>
#include <Vcl.Menus.hpp>
//---------------------------------------------------------------------------
class TForm2 : public TForm
{
__published:	// IDE-managed Components
	TEdit *Edit1;
	TEdit *Edit2;
	TEdit *Edit3;
	TButton *Button1;
	TMainMenu *MainMenu1;
	TStringGrid *StringGrid1;
	TMenuItem *matrixcomplet1;
	TMenuItem *N11;
	TButton *Button2;
	TMenuItem *N21;
	TMenuItem *N31;
	TMenuItem *N41;
	TMenuItem *N51;
	TMenuItem *N61;
	TMenuItem *N71;
	TMenuItem *N81;
	TMenuItem *N91;
	TMenuItem *N101;
	TMenuItem *N111;
	TMenuItem *N121;
	TMenuItem *N131;
	TMenuItem *N141;
	TMenuItem *N151;
	TMenuItem *N161;
	TMenuItem *N171;
	TMenuItem *N181;
	TMenuItem *N191;
	TMenuItem *N201;
	TMenuItem *N211;
	TMenuItem *N221;
	TMenuItem *N231;
	TMenuItem *N142;
	TMenuItem *N251;
	TMenuItem *triangular1;
	TMenuItem *N12;
	TMenuItem *N22;
	TMenuItem *N261;
	TMenuItem *N32;
	TMenuItem *N42;
	TMenuItem *N52;
	TMenuItem *N62;
	TMenuItem *N72;
	TMenuItem *N82;
	TMenuItem *N92;
	TMenuItem *N102;
	TMenuItem *N112;
	TMenuItem *N122;
	TMenuItem *N132;
	TMenuItem *N143;
	TMenuItem *N152;
	TMenuItem *N162;
	TMenuItem *N172;
	TMenuItem *N182;
	TMenuItem *N192;
	TMenuItem *N202;
	TMenuItem *N212;
	TMenuItem *N222;
	TMenuItem *N232;
	TMenuItem *N144;
	TMenuItem *N252;
	TMenuItem *N262;
	TMenuItem *N271;
	TMenuItem *cadMtx1;
	TMenuItem *N13;
	TMenuItem *N23;
	TMenuItem *cadamatrix1;
	TMenuItem *N33;
	TMenuItem *N43;
	TMenuItem *N53;
	TMenuItem *N63;
	TMenuItem *llenarde1losvacios1;
	TMenuItem *vectores1;
	TMenuItem *N14;
	TMenuItem *cantidad1;
	TMenuItem *recorrer1;
	TMenuItem *anadirenp1;
	TMenuItem *eliminar1;
	TMenuItem *invertir1;
	TMenuItem *ordenar1;
	TMenuItem *cargarRND1;
	TMenuItem *cargarveccondig1001;
	TMenuItem *devecatnuminvertido1;
	TMenuItem *cadenas1;
	TMenuItem *invertir2;
	TMenuItem *eliminarmdatosdeunap1;
	TMenuItem *movermulde3alfinal1;
	TMenuItem *insertar1vecaotroenp1;
	TStringGrid *StringGrid2;
	TMenuItem *cargar2RND1;
	TMenuItem *sumadelsegmetno1;
	TMenuItem *cadavector1;
	TMenuItem *pasarprimeraletradelvectoracadena1;
	TMenuItem *verifpalindromo1;
	TMenuItem *insertar1letraenunap1;
	TMenuItem *verifpalindrome1;
	TMenuItem *denumacadrepnum1;
	TMenuItem *numeros1;
	TMenuItem *sumabinarios1;
	TMenuItem *sumaoctales1;
	TMenuItem *factorial1;
	void __fastcall Button1Click(TObject *Sender);
	void __fastcall Button2Click(TObject *Sender);
	void __fastcall N11Click(TObject *Sender);
	void __fastcall N21Click(TObject *Sender);
	void __fastcall N31Click(TObject *Sender);
	void __fastcall N41Click(TObject *Sender);
	void __fastcall N51Click(TObject *Sender);
	void __fastcall N61Click(TObject *Sender);
	void __fastcall N71Click(TObject *Sender);
	void __fastcall N81Click(TObject *Sender);
	void __fastcall N91Click(TObject *Sender);
	void __fastcall N101Click(TObject *Sender);
	void __fastcall N111Click(TObject *Sender);
	void __fastcall N121Click(TObject *Sender);
	void __fastcall N131Click(TObject *Sender);
	void __fastcall N141Click(TObject *Sender);
	void __fastcall N151Click(TObject *Sender);
	void __fastcall N161Click(TObject *Sender);
	void __fastcall N171Click(TObject *Sender);
	void __fastcall N181Click(TObject *Sender);
	void __fastcall N191Click(TObject *Sender);
	void __fastcall N201Click(TObject *Sender);
	void __fastcall N211Click(TObject *Sender);
	void __fastcall N221Click(TObject *Sender);
	void __fastcall N231Click(TObject *Sender);
	void __fastcall N142Click(TObject *Sender);
	void __fastcall N251Click(TObject *Sender);
	void __fastcall N12Click(TObject *Sender);
	void __fastcall N22Click(TObject *Sender);
	void __fastcall N261Click(TObject *Sender);
	void __fastcall N32Click(TObject *Sender);
	void __fastcall N42Click(TObject *Sender);
	void __fastcall N52Click(TObject *Sender);
	void __fastcall N62Click(TObject *Sender);
	void __fastcall N72Click(TObject *Sender);
	void __fastcall N82Click(TObject *Sender);
	void __fastcall N92Click(TObject *Sender);
	void __fastcall N102Click(TObject *Sender);
	void __fastcall N112Click(TObject *Sender);
	void __fastcall N122Click(TObject *Sender);
	void __fastcall N132Click(TObject *Sender);
	void __fastcall N143Click(TObject *Sender);
	void __fastcall N152Click(TObject *Sender);
	void __fastcall N162Click(TObject *Sender);
	void __fastcall N172Click(TObject *Sender);
	void __fastcall N182Click(TObject *Sender);
	void __fastcall N192Click(TObject *Sender);
	void __fastcall N202Click(TObject *Sender);
	void __fastcall N212Click(TObject *Sender);
	void __fastcall N222Click(TObject *Sender);
	void __fastcall N232Click(TObject *Sender);
	void __fastcall N144Click(TObject *Sender);
	void __fastcall N252Click(TObject *Sender);
	void __fastcall N262Click(TObject *Sender);
	void __fastcall N271Click(TObject *Sender);
	void __fastcall N13Click(TObject *Sender);
	void __fastcall N23Click(TObject *Sender);
	void __fastcall cadamatrix1Click(TObject *Sender);
	void __fastcall N33Click(TObject *Sender);
	void __fastcall N43Click(TObject *Sender);
	void __fastcall N53Click(TObject *Sender);
	void __fastcall N63Click(TObject *Sender);
	void __fastcall llenarde1losvacios1Click(TObject *Sender);
	void __fastcall N14Click(TObject *Sender);
	void __fastcall cargarRND1Click(TObject *Sender);
	void __fastcall cantidad1Click(TObject *Sender);
	void __fastcall recorrer1Click(TObject *Sender);
	void __fastcall anadirenp1Click(TObject *Sender);
	void __fastcall eliminar1Click(TObject *Sender);
	void __fastcall invertir1Click(TObject *Sender);
	void __fastcall ordenar1Click(TObject *Sender);
	void __fastcall cargarveccondig1001Click(TObject *Sender);
	void __fastcall devecatnuminvertido1Click(TObject *Sender);
	void __fastcall invertir2Click(TObject *Sender);
	void __fastcall eliminarmdatosdeunap1Click(TObject *Sender);
	void __fastcall movermulde3alfinal1Click(TObject *Sender);
	void __fastcall insertar1vecaotroenp1Click(TObject *Sender);
	void __fastcall cargar2RND1Click(TObject *Sender);
	void __fastcall sumadelsegmetno1Click(TObject *Sender);
	void __fastcall cadavector1Click(TObject *Sender);
	void __fastcall pasarprimeraletradelvectoracadena1Click(TObject *Sender);
	void __fastcall verifpalindromo1Click(TObject *Sender);
	void __fastcall insertar1letraenunap1Click(TObject *Sender);
	void __fastcall verifpalindrome1Click(TObject *Sender);
	void __fastcall denumacadrepnum1Click(TObject *Sender);
	void __fastcall sumabinarios1Click(TObject *Sender);
	void __fastcall sumaoctales1Click(TObject *Sender);
	void __fastcall factorial1Click(TObject *Sender);
private:	// User declarations
public:		// User declarations
	__fastcall TForm2(TComponent* Owner);
};
//---------------------------------------------------------------------------
extern PACKAGE TForm2 *Form2;
//---------------------------------------------------------------------------
#endif
